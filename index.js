 ...
 * Fixed: Status View/React logic for 2026 Protocols
 * Stability: High-Performance Caching Enabled
 */
...
async function loadGiftedSession() {
    ...
    if (config.SESSION_ID && config.SESSION_ID.startsWith("POPKID~")) {
        try {
            ...
            cmdLogger.success("Session decrypted and restored! ✅");
            return true;
...
async function connectToWA() {
    try {
        ...
        conn = makeWASocket({
            ...
            auth: {
                creds: state.creds,
                // FIX: caching signal keys prevents high CPU usage and lags
                keys: makeCacheableSignalKeyStore(state.keys, P({ level: 'silent' })) 
            },
            version,
            // FIX: syncFullHistory=true causes bots to freeze while downloading history
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                cmdLogger.success('POPKID-MD IS LIVE 🚀');
                
                // Startup Alert
                let up = `╔══════════════════╗\n║ 🚀 POPKID-MD CONNECTED\n╠══════════════════╣\n║ 👤 USER: ${conn.user.name || 'Bot'}\n║ 🔑 PREFIX: ${config.PREFIX}\n║ 👨‍💻 STATUS: Viewing & Reacting ✅\n╚══════════════════╝`;
        ...
        conn.ev.on('messages.upsert', async (mek) => {
            ...
            // ============ ⚡ THE STATUS MASTER FIX ============
            if (isStatus) {
                // Get the exact person who posted the status
                ...
                if (config.AUTO_STATUS_REACT === "true" && !msg.key.fromMe) {
                    ...
                    // FIX: Must include participant and myJid in statusJidList
                ...
                return; // Stop processing status broadcast to save speed
            }
            ...
            // Filter for Mode (Public/Private)
